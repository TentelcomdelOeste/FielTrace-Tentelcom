...
export const cameraService = {
   ...
   * The processed image is passed directly as a data URL; no Cache/temp file is used
   * from the app layer (the plugin may create a short-lived temp internally for decode).
   */
   ...
   * On Android the identifier must belong to the app's album path (getAlbumsPath).
   * Identifier is cached for the process lifetime to avoid repeated MediaStore queries.
   */
      ...
      try {
        ...
        request.onsuccess = () => {
          ...
          addRequest.onsuccess = () => {
            console.log(`[IndexedDB] Foto guardada: ${fileName}`);
            resolve(true);
          };
