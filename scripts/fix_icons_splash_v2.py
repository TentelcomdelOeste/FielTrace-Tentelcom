#!/usr/bin/env python3
"""Launcher a bit more padded; splash uses 512 HD with slight pull-back. Resources only."""
...
if src.name != "android-chrome-512x512.png":
    dest = PUBLIC / "android-chrome-512x512.png"
...
# Slightly pulled back so bottom-right tentelcom is visible
LAUNCHER_RATIO = 0.68
for folder, size in {
    "mipmap-mdpi": 108,
    "mipmap-hdpi": 162,
    ...
    d = RES / folder
    d.mkdir(parents=True, exist_ok=True)
    padded(img, size, LAUNCHER_RATIO).save(d / "ic_launcher_foreground.png", "PNG")
...
<resources>
    <color name="ic_launcher_background">#003499</color>
</resources>
""",
    encoding="utf-8",
)
...
print(f"OK launcher_ratio={LAUNCHER_RATIO} splash_ratio={SPLASH_RATIO} src={src}")
